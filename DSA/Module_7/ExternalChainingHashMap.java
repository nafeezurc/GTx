...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if(!exists) {
            size++;
            double currLF = (double) size / table.length;
    ...
    public V remove(K key) {
        ...
        int hc = key.hashCode();
        int index = Math.abs(hc % table.length);
        if(table[index] == null) throw new NoSuchElementException("Key not in table.");
        V res = null;
        ...
        if(!table[index].getKey().equals(key)) {
            ...
            while(currNode.getNext() != null) {
                if(currNode.getNext().getKey().equals(key)) {
                    exists = true;
                    size--;
                    res = currNode.getNext().getValue();
                    currNode.setNext(currNode.getNext().getNext());
                    break;
                }
                currNode = currNode.getNext();
            }
            if(!exists) throw new NoSuchElementException("Key not in table.");
        }
        else {
            size--;
    ...
    private void resizeBackingTable(int length) {
        ...
        for(ExternalChainingMapEntry<K, V> entry : table) {
            ...
            ExternalChainingMapEntry<K, V> currNode = entry;
            while(currNode != null) {
                int newHC = currNode.getKey().hashCode();
                int newIndex = Math.abs(newHC % length);
                ExternalChainingMapEntry<K, V> newNode = new ExternalChainingMapEntry<>(currNode.getKey(), currNode.getValue());
                if(newTable[newIndex] != null) {
                    newNode.setNext(newTable[newIndex]);
                    newTable[newIndex] = newNode;
                }
                else newTable[newIndex] = newNode;
                currNode = currNode.getNext();
            }
        }
